...
- [ ] 到本阶段再生成站点图标概念图，供人工查看是否合适。
- [ ] 图标要体现个人识别度、稳定感和记忆点。
- [ ] 图标可以从姓名含义、博客气质、知识库属性或个人偏好中提取符号，但文档不固定具体姓名。
- [ ] 图标整体要高级、克制、有深意，不做直白廉价皇冠。
- [ ] 图标避免明显 AI 味，不使用混乱纹理、过度发光、无意义复杂细节。
- [ ] 图标候选图先作为预览，不直接替换 favicon。
- [ ] 如果候选图不满意，记录不满意原因，例如太商业、太普通、太复杂、太像游戏图标。

### 图标资产整理

- [ ] 选定图标后放入 `public/icons/`。
- [ ] 保留原始大图，例如 `public/icons/site-icon-source.png`。
- [ ] 导出 `favicon.ico`。
- [ ] 导出 `icon-32.png`。
- [ ] 导出 `icon-192.png`。
- [ ] 导出 `apple-touch-icon.png`。
- [ ] 深色背景和浅色背景各检查一次图标识别度。
- [ ] 小尺寸 16px、32px 下检查是否仍能识别主要形状。
- [ ] 如果图标包含文字，确认小尺寸不会糊成噪点；不适合时改为抽象符号。

### 站点接入

- [ ] 在 `nuxt.config.ts` 中配置 favicon。
- [ ] 在 `nuxt.config.ts` 中配置 Apple touch icon。
- [ ] 在 `app/config/site.ts` 中维护图标路径。
- [ ] Header 左侧可以使用图标 + 站点名，移动端可只显示图标或短名称。
- [ ] 图标在白天模式、黑夜模式和背景图上都保持清晰。
- [ ] 图标不和小狗助手、音乐播放器视觉风格冲突。

### 阶段验收

- [ ] 图标能体现个人识别度，不是随机生成的抽象图。
- [ ] 图标有深意但不过度解释。
- [ ] 图标小尺寸可识别。
- [ ] favicon 在浏览器标签页显示正常。
- [ ] 图标在浅色和深色模式下都不突兀。
...
- [ ] 桌面端 Header 不遮挡首页首屏标题。
- [ ] 移动端 Header 不遮挡内容。
- [ ] Footer 在内容少时贴近底部，在内容多时自然跟随。
- [ ] 主题切换或设置入口位置稳定。
