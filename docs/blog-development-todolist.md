...
- [x] 点击小狗菜单的音乐播放可以打开播放器。
- [x] APlayer/MetingJS 能读取配置。
- [x] QQ 音乐歌单 ID 填入后能尝试解析。
- [x] 播放器不会遮挡正文和小狗。
- [x] 深色模式下播放器不突兀。
...
阶段 12 进展记录（2026-05-16）：
- 宠物移动端缩小至 80×80px（PC 保持 136px），底部位置避开音乐播放器（`bottom: 72px`）。
- 宠物菜单新增隐藏按钮，点击后收起为边缘小图标，可点击恢复。
- 修复触控目标：ArticleToc、WikiSideToc、BlogFilterBar 切换/清除按钮、BlogFilterBar 标签、WikiTree 分类、TodoBoard 列头、TodoItem 均调整至 ≥44px。
- 补充 `aria-expanded`（ArticleToc、WikiSideToc）、`aria-controls`（BlogFilterBar）、`aria-label`（WikiTree）。
- 首页添加 `overflow-x: hidden` 防止负边距导致横向溢出。
- 各项断点检查：980px（TOC 折叠）、860px（列表单列）、700px（标题换行、筛选折叠）、560px（统计堆叠）均已验证。
